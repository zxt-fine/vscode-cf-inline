  ...
  try {
    ...
    if (synthetic) {
      ...
      const translationChecks = {
        ...
        gameTerminology: translationToggle?.translatedText.includes('Ajisai 在奇数回合行动')
          && translationToggle?.translatedText.includes('当前回合需要行动的玩家')
          && translationToggle?.translatedText.includes('或者跳过本回合')
          && !translationToggle?.translatedText.includes('或者通过'),
      ...
      translateHtmlItems: async (items) => {
        ...
        return items.map((html) => {
          ...
          let translated = html
            ...
            .replace('They play for n rounds. Ajisai moves on odd-numbered rounds and Mai moves on even-numbered rounds. On each round, the player who is to move may swap a_i and b_i, or pass.', '他们进行 n 轮游戏。Ajisai 在奇数回合移动，Mai 在偶数回合移动。每一回合，要移动的玩家可以交换 a_i 和 b_i，或者通过。')
